 ...
 *
 * The slab spans the hero, but its contents do not: past the width the row
 * needs, the three groups are held to a fixed rail and centred. Without that
 * they keep travelling to the outer edges, and on a wide monitor the bar reads
 * as a thin blue line with the details stranded at either end — this way the
 * arrangement is the same at 1280 as it is at 2560, which is the point.
 */
export function HeroContactBar() {
  return (
    <div className="mt-3 w-full rounded-[1.5rem] bg-brand-600 px-4 py-3 text-white sm:rounded-pill sm:px-5 sm:py-1.5">
      <div className="flex w-full flex-col items-center gap-3 sm:flex-row sm:justify-between sm:gap-6">
        <a
