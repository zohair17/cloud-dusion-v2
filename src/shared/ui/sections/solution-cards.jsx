          ...
          <ParallaxCard key={item.slug} depth={index % 2 === 0 ? 0.55 : 0.25} className="h-full">
            ...
            >
              <div className="flex flex-1 flex-col px-7 pb-6 pt-7">
                <span className="inline-flex h-13 w-13 items-center justify-center rounded-2xl bg-[linear-gradient(140deg,var(--color-brand-500),var(--color-brand-700))] p-3.5 text-white shadow-[0_12px_26px_-10px_rgb(53_51_205/0.65)] ring-1 ring-white/25 transition-transform duration-500 group-hover:-translate-y-0.5">
                  <Icon className="h-6 w-6" aria-hidden="true" />
                </span>

                <h3 className="mt-5 font-display text-xl font-semibold leading-tight tracking-tight text-balance">
                  {item.title}
                </h3>
                <p className="mt-2.5 flex-1 text-sm leading-relaxed text-muted">{item.summary}</p>
